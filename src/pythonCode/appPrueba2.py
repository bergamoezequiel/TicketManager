...
app = Flask(__name__)
app.config['SERVER_NAME'] = '172.22.16.41:5000'
api = Api(app)
...
class User(Resource):
    ...
    def post(self, name):
        ...
        user = {
            ...
            "occupation": request.headers.get('occupation')
        }
        users.append(user)
        c.execute('INSERT INTO Personas VALUES (?,?,?)',(name,request.headers.get('age'),request.headers.get('occupation')))
        conn.commit()
    ...
    def put(self, name):
        ...
        for user in users:
            if(name == user["name"]):
                user["age"] = request.headers.get('age')
                user["occupation"] = request.headers.get('occupation')
                return user, 200
